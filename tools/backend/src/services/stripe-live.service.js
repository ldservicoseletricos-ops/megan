...
function getConfig() {
  return { mode: process.env.STRIPE_SECRET_KEY ? 'live_ready' : 'simulation', webhookReady: Boolean(process.env.STRIPE_WEBHOOK_SECRET) };
}
function listCheckouts() { return readJson(checkoutsPath, []); }
...
module.exports = { getConfig, listCheckouts, createCheckoutSession, markPaid, processWebhook };
