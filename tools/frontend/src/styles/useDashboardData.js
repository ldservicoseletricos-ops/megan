...
export function useDashboardData() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [envResponse, guideResponse, finalResponse, autoFixResponse, actionResponse] = await Promise.all([
        ...
        console.error('Erro ao carregar Megan V17', error);
      } finally {
        setLoading(false);
      }
    }
  ...
  return { envCheck, deployGuide, finalCheck, autoFix, actionPriority, loading };
}
